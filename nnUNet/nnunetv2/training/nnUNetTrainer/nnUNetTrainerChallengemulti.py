...
class CrossAttentionPooling(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        B, C, D, H, W = x.shape
        N = D * H * W
        feat = x.view(B, C, N).transpose(1, 2)  # [B, N, C]
...
class nnUNetTrainerChallengemulti(nnUNetTrainer):
    alpha_cls: float = 2.0
    ...
    def _cls_loss(self, logits: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
    ...
    # ---------- save and load heads ----------
    def _heads_meta(self):
        return {
            "embed_dim": getattr(self._cap, "embed_dim", None),
            "query_num": getattr(self._cap, "query_num", None),
            "num_classes": 3,
            "cls_dropout_p": self.cls_dropout_p,
        }

    def _save_cls_heads(self, tag_stem: str):
        """Save CAP and cls_head next to nnU-Net checkpoints."""
        out_dir = Path(self.output_folder)
        out_dir.mkdir(parents=True, exist_ok=True)
        fn = out_dir / f"{tag_stem}_cls_heads.pth"
        payload = {
            "cap": self._cap.state_dict(),
            "cls_head": self.cls_head.state_dict(),
            "meta": self._heads_meta(),
        }
        torch.save(payload, fn)
        print(f"Saved classification heads -> {fn}")

    def _try_load_cls_heads(self):
        """
        When resuming, try to load heads from one of:
          checkpoint_latest_cls_heads.pth
          checkpoint_best_cls_heads.pth
          checkpoint_final_cls_heads.pth
        """
        out_dir = Path(self.output_folder)
        cands = [
            out_dir / "checkpoint_latest_cls_heads.pth",
            out_dir / "checkpoint_best_cls_heads.pth",
            out_dir / "checkpoint_final_cls_heads.pth",
        ]
        for fn in cands:
            if fn.exists():
                try:
                    sd = torch.load(str(fn), map_location="cpu")
                    self._cap.load_state_dict(sd["cap"], strict=True)
                    self.cls_head.load_state_dict(sd["cls_head"], strict=True)
                    print(f"Loaded classification heads from {fn.name}")
                    return True
                except Exception as e:
                    print(f"Could not load heads from {fn.name}: {e}")
        return False
    ...
    def on_train_start(self):
        ...
        try:
            self._wb = wandb.init(
                project=os.environ.get("WANDB_PROJECT", "nnUNet_Challenge_multi"),
    ...
    def on_iteration_end(self):
        ...
        try:
            ...
            if (
                self._last_seg_loss is not None
                and self._last_cls_loss is not None
                and self._last_total_loss is not None
                and it % 10 == 0
            ):
                if self._wb:
                    wandb.log(
                ...
                if it % 100 == 0:
                    print(
                        f"Iter {it}: seg_loss={self._last_seg_loss:.4f}, cls_loss={self._last_cls_loss:.4f}, total={self._last_total_loss:.4f}"
                    )
        except Exception as e:
            pass
    ...
    def on_epoch_end(self):
        ...
        if logs:
            try:
                ...
                print(f"Logged {len(logs)} metrics to W&B")
            except Exception as e:
    ...
    def on_train_end(self):
        print("=== Final validation and saving classification heads ===")
        ...
        try:
            self._save_cls_heads("checkpoint_final")
        except Exception as e:
            print(f"Saving heads failed: {e}")

        if final_logs and self._wb:
            try:
                wandb.log(final_logs, step=int(_safe(self, "current_epoch", -1)))
                print("Logged final validation metrics to W&B")
            except Exception as e:
    ...
    def save_checkpoint(self, filename: str) -> None:
        """
        Called by nnU-Net when saving best/latest/final. We piggyback to save heads too.
        Signature must match nnUNetTrainer.save_checkpoint(self, filename: str).
        """
        ...
        try:
            ...
            self._save_cls_heads(stem)
        except Exception as e:
            print(f"Saving heads with tag {filename} failed: {e}")


    def load_checkpoint_ram(self, checkpoint: dict, train: bool = True):
        """
        Called by nnU-Net when resuming from RAM checkpoint.
        After the regular load, try to load our heads from sidecar files in output_folder.
        Signature must match base class.
        """
        ...
        try:
            ...
            print(f"Loading heads on resume failed: {e}")
